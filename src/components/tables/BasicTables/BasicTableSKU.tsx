...
export default function BasicTableSKU() {
  ...
  const [selectedData, setSelectedData] = useState<SKU | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        "http://127.0.0.1:8000/api/sku/filter",
        {
          headers: {
...
useEffect(() => {
  ...
  // === HANDLE UNTUK GET DATA SKU SETIAP ROLE ===
  const fetchData = async () => {
    try {
       setLoading(true);
    setData([]); // ⛔ cegah render data lama
      ...
      switch (user_type) {
        case "rt":
          apiURL = "http://127.0.0.1:8000/api/rt/sku/";
          break;
        case "perangkat_desa":
          apiURL = "http://127.0.0.1:8000/api/perangkat_desa/sku/";
          break;
        case "kepala_desa":
          apiURL = "http://127.0.0.1:8000/api/kepala_desa/sku/";
          break;
        case "super_admin":
          apiURL = "http://127.0.0.1:8000/api/super_admin/sku/";
          break;
        default:
          apiURL = "http://127.0.0.1:8000/api/masyarakat/sku/my";
          break;
      }
      ...
      console.error("Gagal mengambil data Surat Keterangan Usaha:", error);
      ...
      setAlertData({
        ...
        message:
          "Tidak dapat mengambil data Surat Keterangan Usaha. Periksa koneksi Anda atau coba beberapa saat lagi.",
      });
 ...
 const handleAjukanKembali = async () => {
  ...
  if (!formUpdate.bidang_usaha) newErrors.bidang_usaha = "Bidang usaha wajib diisi";
  ...
  fd.append("bidang_usaha", formUpdate.bidang_usaha);
  ...
  try {
    const res = await axios.post(
      `http://127.0.0.1:8000/api/masyarakat/sku/${selectedData.id_sku}`,
      fd,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const openModalDetail = (item: SKU) => {
    setSelectedData(item);
    setModalOpen(true);
  };

  const openModalUpdate = (item: SKU) => {
  setSelectedData(item);
  setUpdateStatus("");
  ...
  const openModalAjukanKembali = (item: SKU) => {
    setSelectedData(item);
  ...
  const getDefaultPoinII = (bidangUsaha?: string) => {
  return `Orang tersebut di atas benar-benar penduduk Desa Mejayan Kecamatan Mejayan Kabupaten Madiun dan orang tersebut diatas benar-benar memiliki usaha dibidang : ${
    bidangUsaha && bidangUsaha.trim() !== ""
      ? bidangUsaha
      : "........"
  }`;
};
  ...
  data.poin_ii && data.poin_ii.trim() !== ""
    ? data.poin_ii
    : getDefaultPoinII(data.bidang_usaha)
);
  ...
  const openModalUpdateKepala = (item: SKU) => {
    setSelectedData(item);
  ...
  if (!selectedData) {
    ...
    try {
      ...
      const id_sku = selectedData.id_sku;
    ...
    const res = await axios.post(
      `http://127.0.0.1:8000/api/rt/${rt_id}/sku/${id_sku}/proses`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    window.location.reload();
      ...
      // refresh data table
      setData((prev) =>
        prev.map((d) => (d.id_sku === id_sku ? res.data.data : d))
      );
  ...
  const handleUpdatePerangkat = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://127.0.0.1:8000/api/perangkat-desa/${perangkat_id}/sku/${id_sku}/proses`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  if (updateStatusKepala === "selesai" && updateTtdKepala) {
    payload.append("ttd_kades", updateTtdKepala);
      ...
      await axios.post(
        `http://127.0.0.1:8000/api/kepala_desa/${kepalaDesaId}/sku/${id_sku}/validasi`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <span className="block text-gray-500 text-theme-xs dark:text-gray-400">
                Pemohon Surat Keterangan Usaha
              </span>
            </div>
          </div>
        </TableCell>
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-2xl">
    ...
    <div className="mb-4 border-b pb-3 dark:border-gray-700">
      <h4 className="text-2xl font-bold text-blue-700 dark:text-blue-400">
        Detail Pengajuan Surat Keterangan Usaha
      </h4>
    </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-8">
              {[
                ...
                { label: "Bidang Usaha", value: selectedData.bidang_usaha },
  ...
  <div className="flex flex-col gap-2.5 pl-1">
    ...
    {selectedData.file_pdf && (
      ...
      >
        ...
        Download Surat Keterangan Usaha Resmi Terverifikasi (PDF)
      </a>
    )}
  </div>
</div>
        ...
        <div className="relative flex flex-col w-full max-h-[90vh] overflow-hidden rounded-3xl bg-white dark:bg-gray-900 shadow-xl">
          ...
          <div className="px-6 pt-8 pb-4 lg:px-11 lg:pt-11">
            <h4 className="mb-2 text-2xl font-bold text-gray-800 dark:text-white/90">
              Ajukan Kembali Surat Keterangan Usaha
            </h4>
            ...
            <div className="custom-scrollbar flex-grow overflow-y-auto px-6 pb-6 lg:px-11">
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    type="text"
                    value={formUpdate.bidang_usaha}
                    onChange={(e) => setFormUpdate({ ...formUpdate, bidang_usaha: e.target.value })}
                  />
      ...
      <div className="mb-4">
        <div className="flex justify-between items-center mb-1.5">
          ...
          <button
            type="button"
            onClick={() =>
  setUpdatePoinII(getDefaultPoinII(selectedData?.bidang_usaha))
}
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-xl">
    ...
    <div className="mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        <button 
          type="button"
          onClick={() => setUpdateKeteranganKepala("Seluruh persyaratan telah terpenuhi dengan benar, dan Surat Keterangan Usaha sudah disahkan. Periksa di detail pengajuan surat untuk mengunduh file.")}
          className="px-2 py-1 text-[11px] bg-green-50 text-green-600 border border-green-200 rounded-md hover:bg-green-100 transition"
        >
          + Data Valid
        </button>
