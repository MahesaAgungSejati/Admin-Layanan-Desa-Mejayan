...
export default function BasicTableSKP() {
  const [data, setData] = useState<SKP[]>([]);
  ...
  const [selectedData, setSelectedData] = useState<SKP | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        "http://127.0.0.1:8000/api/skp/filter",
        {
          headers: {
...
useEffect(() => {
  ...
  // === HANDLE UNTUK GET DATA SKP SETIAP ROLE ===
  const fetchData = async () => {
    try {
       setLoading(true);
    setData([]); // ⛔ cegah render data lama
      ...
      switch (user_type) {
        case "rt":
          apiURL = "http://127.0.0.1:8000/api/rt/skp/";
          break;
        case "perangkat_desa":
          apiURL = "http://127.0.0.1:8000/api/perangkat_desa/skp/";
          break;
        case "kepala_desa":
          apiURL = "http://127.0.0.1:8000/api/kepala_desa/skp/";
          break;
        case "super_admin":
          apiURL = "http://127.0.0.1:8000/api/super_admin/skp/";
          break;
        default:
          apiURL = "http://127.0.0.1:8000/api/masyarakat/skp/my";
          break;
      }
      ...
      console.error("Gagal mengambil data Surat Keterangan Penduduk:", error);
      ...
      setAlertData({
        ...
        message:
          "Tidak dapat mengambil data Surat Keterangan Penduduk. Periksa koneksi Anda atau coba beberapa saat lagi.",
      });
 ...
 const handleAjukanKembali = async () => {
  ...
  try {
    const res = await axios.post(
      `http://127.0.0.1:8000/api/masyarakat/skp/${selectedData.id_skp}`,
      fd,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const openModalDetail = (item: SKP) => {
    setSelectedData(item);
    setModalOpen(true);
  };

  const openModalUpdate = (item: SKP) => {
  setSelectedData(item);
  setUpdateStatus("");
  ...
  const openModalAjukanKembali = (item: SKP) => {
    setSelectedData(item);
  ...
  const openModalUpdateKepala = (item: SKP) => {
    setSelectedData(item);
  ...
  if (!selectedData) {
    ...
    try {
      ...
      const id_skp = selectedData.id_skp;
    ...
    const res = await axios.post(
      `http://127.0.0.1:8000/api/rt/${rt_id}/skp/${id_skp}/proses`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    window.location.reload();
      ...
      // refresh data table
      setData((prev) =>
        prev.map((d) => (d.id_skp === id_skp ? res.data.data : d))
      );
  ...
  const handleUpdatePerangkat = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://127.0.0.1:8000/api/perangkat-desa/${perangkat_id}/skp/${id_skp}/proses`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  if (updateStatusKepala === "selesai" && updateTtdKepala) {
    payload.append("ttd_kades", updateTtdKepala);
      ...
      await axios.post(
        `http://127.0.0.1:8000/api/kepala_desa/${kepalaDesaId}/skp/${id_skp}/validasi`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <span className="block text-gray-500 text-theme-xs dark:text-gray-400">
                Pemohon Surat Keterangan Penduduk
              </span>
            </div>
          </div>
        </TableCell>
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-2xl">
    ...
    <div className="mb-4 border-b pb-3 dark:border-gray-700">
      <h4 className="text-2xl font-bold text-blue-700 dark:text-blue-400">
        Detail Pengajuan Surat Keterangan Penduduk
      </h4>
    </div>
  ...
  <div className="flex flex-col gap-2.5 pl-1">
    ...
    {selectedData.file_pdf && (
      ...
      >
        ...
        Download Surat Keterangan Penduduk Resmi Terverifikasi (PDF)
      </a>
    )}
  </div>
</div>
        ...
        <div className="relative flex flex-col w-full max-h-[90vh] overflow-hidden rounded-3xl bg-white dark:bg-gray-900 shadow-xl">
          ...
          <div className="px-6 pt-8 pb-4 lg:px-11 lg:pt-11">
            <h4 className="mb-2 text-2xl font-bold text-gray-800 dark:text-white/90">
              Ajukan Kembali Surat Keterangan Penduduk
            </h4>
              ...
              {/* KEWARGANEGARAAN */}
                ...
                <div className="lg:col-span-2 space-y-2">
                  ...
                  <TextArea
                    ...
                    placeholder="Berikan alasan perbaikan data..."
                  />
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-xl">
    ...
    <div className="mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        <button 
          type="button"
          onClick={() => setUpdateKeteranganKepala("Seluruh persyaratan telah terpenuhi dengan benar, dan Surat Keterangan Penduduk sudah disahkan. Periksa di detail pengajuan surat untuk mengunduh file.")}
          className="px-2 py-1 text-[11px] bg-green-50 text-green-600 border border-green-200 rounded-md hover:bg-green-100 transition"
        >
          + Data Valid
        </button>
