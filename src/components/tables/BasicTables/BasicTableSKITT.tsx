...
export default function BasicTableSKITT() {
  const [data, setData] = useState<SKITT[]>([]);
  ...
  const [selectedData, setSelectedData] = useState<SKITT | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        "http://127.0.0.1:8000/api/skitt/filter",
        {
          headers: {
...
useEffect(() => {
  ...
  // === HANDLE UNTUK GET DATA SKITT SETIAP ROLE ===
  const fetchData = async () => {
    try {
       setLoading(true);
    setData([]); // ⛔ cegah render data lama
      ...
      switch (user_type) {
        case "rt":
          apiURL = "http://127.0.0.1:8000/api/rt/skitt/";
          break;
        case "perangkat_desa":
          apiURL = "http://127.0.0.1:8000/api/perangkat_desa/skitt/";
          break;
        case "kepala_desa":
          apiURL = "http://127.0.0.1:8000/api/kepala_desa/skitt/";
          break;
        case "super_admin":
          apiURL = "http://127.0.0.1:8000/api/super_admin/skitt/";
          break;
        default:
          apiURL = "http://127.0.0.1:8000/api/masyarakat/skitt/my";
          break;
      }
      ...
      console.error("Gagal mengambil data Surat Keterangan Imunisasi TT:", error);
      ...
      setAlertData({
        ...
        message:
          "Tidak dapat mengambil data Surat Keterangan Imunisasi TT. Periksa koneksi Anda atau coba beberapa saat lagi.",
      });
 ...
 const handleAjukanKembali = async () => {
  ...
  if (!formUpdate.fasilitas_kesehatan) newErrors.fasilitas_kesehatan = "Nama fasilitas kesehatan wajib diisi";
...
if (!formUpdate.file_ktp) newErrors.file_ktp = "KTP wajib tersedia";
  ...
  Object.keys(formUpdate).forEach((key) => {
    if (!["file_ktp", "file_kk", "ttd_masyarakat", "file"].includes(key)) {
      fd.append(key, (formUpdate as any)[key]);
    }
  });
  ...
  fd.append("fasilitas_kesehatan", formUpdate.fasilitas_kesehatan);
 ...
 if (formUpdate.file_ktp instanceof File) {
  ...
  try {
    const res = await axios.post(
      `http://127.0.0.1:8000/api/masyarakat/skitt/${selectedData.id_skitt}`,
      fd,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const openModalDetail = (item: SKITT) => {
    setSelectedData(item);
    setModalOpen(true);
  };

  const openModalUpdate = (item: SKITT) => {
  setSelectedData(item);
  setUpdateStatus("");
  ...
  const openModalAjukanKembali = (item: SKITT) => {
    setSelectedData(item);
  ...
  setFormUpdate({
    ...
    alasan: item.alasan ?? "",
    fasilitas_kesehatan: item.fasilitas_kesehatan ?? "",
    keterangan: "",
  ...
  const DEFAULT_POIN_II = `Orang tersebut di atas benar-benar penduduk Desa Mejayan Kecamatan Mejayan Kabupaten Madiun`;
  ...
  const openModalUpdateKepala = (item: SKITT) => {
    setSelectedData(item);
  ...
  if (!selectedData) {
    ...
    try {
      ...
      const id_skitt = selectedData.id_skitt;
    ...
    const res = await axios.post(
      `http://127.0.0.1:8000/api/rt/${rt_id}/skitt/${id_skitt}/proses`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    window.location.reload();
      ...
      // refresh data table
      setData((prev) =>
        prev.map((d) => (d.id_skitt === id_skitt ? res.data.data : d))
      );
  ...
  const handleUpdatePerangkat = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://127.0.0.1:8000/api/perangkat-desa/${perangkat_id}/skitt/${id_skitt}/proses`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  if (updateStatusKepala === "selesai" && updateTtdKepala) {
    payload.append("ttd_kades", updateTtdKepala);
      ...
      await axios.post(
        `http://127.0.0.1:8000/api/kepala_desa/${kepalaDesaId}/skitt/${id_skitt}/validasi`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <span className="block text-gray-500 text-theme-xs dark:text-gray-400">
                Pemohon Surat Keterangan Imunisasi TT
              </span>
            </div>
          </div>
        </TableCell>
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-2xl">
    ...
    <div className="mb-4 border-b pb-3 dark:border-gray-700">
      <h4 className="text-2xl font-bold text-blue-700 dark:text-blue-400">
        Detail Pengajuan Surat Keterangan Imunisasi TT
      </h4>
    </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-8">
              {[
                ...
                { label: "Fasilitas Kesehatan", value: selectedData.fasilitas_kesehatan },
  ...
  <div className="flex flex-col gap-2.5 pl-1">
    ...
    {selectedData.file_pdf && (
      ...
      >
        ...
        Download Surat Keterangan Imunisasi TT Resmi Terverifikasi (PDF)
      </a>
    )}
  </div>
</div>
        ...
        <div className="relative flex flex-col w-full max-h-[90vh] overflow-hidden rounded-3xl bg-white dark:bg-gray-900 shadow-xl">
          ...
          <div className="px-6 pt-8 pb-4 lg:px-11 lg:pt-11">
            <h4 className="mb-2 text-2xl font-bold text-gray-800 dark:text-white/90">
              Ajukan Kembali Surat Keterangan Imunisasi TT
            </h4>
            ...
            <div className="custom-scrollbar flex-grow overflow-y-auto px-6 pb-6 lg:px-11">
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div className="space-y-2">
                  <Label>Nama</Label>
                  <Input type="text" value={formUpdate.nama} onChange={(e) => setFormUpdate({ ...formUpdate, nama: e.target.value })} />
                  {errors.nama && <p className="mt-1 text-xs text-red-500">{errors.nama}</p>}
                </div>

                <div className="space-y-2">
                  <Label>NIK</Label>
                  <Input type="text" value={formUpdate.nik} onChange={(e) => setFormUpdate({ ...formUpdate, nik: e.target.value })} />
                ...
                <div className="space-y-2">
                  ...
                  <Input type="text" value={formUpdate.ttl} onChange={(e) => setFormUpdate({ ...formUpdate, ttl: e.target.value })} />
                  {errors.ttl && <p className="mt-1 text-xs text-red-500">{errors.ttl}</p>}
                </div>

                <div className="space-y-2">
                  <Label>Status Perkawinan</Label>
                  <Input type="text" value={formUpdate.status_perkawinan} onChange={(e) => setFormUpdate({ ...formUpdate, status_perkawinan: e.target.value })} />
                  {errors.status_perkawinan && <p className="mt-1 text-xs text-red-500">{errors.status_perkawinan}</p>}
                </div>

                <div className="space-y-2">
                  <Label>Agama</Label>
                  <Input type="text" value={formUpdate.agama} onChange={(e) => setFormUpdate({ ...formUpdate, agama: e.target.value })} />
                  {errors.agama && <p className="mt-1 text-xs text-red-500">{errors.agama}</p>}
                </div>

                <div className="space-y-2">
                  <Label>Kewarganegaraan</Label>
                  <Input type="text" value={formUpdate.kewarganegaraan} onChange={(e) => setFormUpdate({ ...formUpdate, kewarganegaraan: e.target.value })} />
                  {errors.kewarganegaraan && <p className="mt-1 text-xs text-red-500">{errors.kewarganegaraan}</p>}
                </div>

                <div className="space-y-2">
                  <Label>Pendidikan</Label>
                  <Input type="text" value={formUpdate.pendidikan} onChange={(e) => setFormUpdate({ ...formUpdate, pendidikan: e.target.value })} />
                  {errors.pendidikan && <p className="mt-1 text-xs text-red-500">{errors.pendidikan}</p>}
                </div>

                <div className="space-y-2">
                  <Label>Pekerjaan</Label>
                  <Input type="text" value={formUpdate.pekerjaan} onChange={(e) => setFormUpdate({ ...formUpdate, pekerjaan: e.target.value })} />
                  {errors.pekerjaan && <p className="mt-1 text-xs text-red-500">{errors.pekerjaan}</p>}
                </div>

                <div className="lg:col-span-2 space-y-2">
                  <Label>Alamat</Label>
                  <Input type="text" value={formUpdate.alamat} onChange={(e) => setFormUpdate({ ...formUpdate, alamat: e.target.value })} />
                  {errors.alamat && <p className="mt-1 text-xs text-red-500">{errors.alamat}</p>}
                </div>

                <div className="lg:col-span-2 space-y-2">
                  <Label>Keperluan</Label>
                  <Input type="text" value={formUpdate.keperluan} onChange={(e) => setFormUpdate({ ...formUpdate, keperluan: e.target.value })} />
                  {errors.keperluan && <p className="mt-1 text-xs text-red-500">{errors.keperluan}</p>}
                </div>

                <div className="lg:col-span-2 space-y-2">
                  <Label>Fasilitas Kesehatan</Label>
                  <TextArea value={formUpdate.fasilitas_kesehatan} onChange={(e) => setFormUpdate({ ...formUpdate, fasilitas_kesehatan: e.target.value })} placeholder="Contoh: Puskesmas Mejayan...." />
                  {errors.fasilitas_kesehatan && <p className="mt-1 text-xs text-red-500">{errors.fasilitas_kesehatan}</p>}
                </div>

                <div className="lg:col-span-2 space-y-2">
                  <Label>Alasan</Label>
                  <TextArea value={formUpdate.alasan} onChange={(e) => setFormUpdate({ ...formUpdate, alasan: e.target.value })} placeholder="Berikan alasan anda mengajukan surat..." />
                  {errors.alasan && <p className="mt-1 text-xs text-red-500">{errors.alasan}</p>}
                </div>
              </div> 
              {/* SELESAI GRID DATA PEMOHON */}
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-xl">
    ...
    <div className="mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        <button 
          type="button"
          onClick={() => setUpdateKeteranganKepala("Seluruh persyaratan telah terpenuhi dengan benar, dan Surat Keterangan Imunisasi TT sudah disahkan. Periksa di detail pengajuan surat untuk mengunduh file.")}
          className="px-2 py-1 text-[11px] bg-green-50 text-green-600 border border-green-200 rounded-md hover:bg-green-100 transition"
        >
          + Data Valid
        </button>
