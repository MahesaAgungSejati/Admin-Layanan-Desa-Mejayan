...
import ModalDetailDataPerangkatDesa from "../../ui/modal/ModalDetailDataPerangkatDesa";
import ModalUpdatePerangkatDesa from "../../ui/modal/ModalUpdatePerangkatDesa";
...
/* ===============================
   COMPONENT
================================ */
export default function TableDataPerangkatDesa() {
...
const fetchData = async () => {
  ...
  try {
    const res = await axios.get(
      "http://127.0.0.1:8000/api/users/perangkat-desa",
      {
        headers: {
    ...
    console.error("Gagal ambil Perangkat Desa:", err);
...
const handleDetail = async (id: number) => {
  try {
    ...
    const res = await axios.get(
      `http://127.0.0.1:8000/api/users/perangkat-desa/${id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    console.error("Gagal mengambil detail Perangkat Desa", error);
  }
};
...
const handleSaveUpdate = async () => {
  ...
  payload.append("email", formData.email);
    ...
    await axios.post(
      `http://127.0.0.1:8000/api/users/perangkat-desa/${selectedId}`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const handleDeletePerangkatDesa = async () => {
  if (!deleteId) return;
  ...
  try {
    ...
    await axios.delete(
      `http://127.0.0.1:8000/api/users/perangkat-desa/${deleteId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        message:
          error?.response?.data?.message ??
          "Gagal menghapus data Perangkat Desa.",
      })
    );
...
<div className="border-t border-gray-200 dark:border-gray-800 -mx-4 sm:-mx-6 px-4 sm:px-6 pt-4 pb-2 mb-2">
                ...
                <TableCell className="px-5 py-4 sm:px-6 text-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200">
                     <img
                        src={perangkat.foto_profil || "/images/user/owner.jpg"}
                        alt={perangkat.nama}
                        onError={(e) => {
  ...
  {/* DETAIL */}
  <button
    onClick={() => handleDetail(perangkat.id)}
  ...
  {/* UPDATE */}
  <button
    onClick={() => handleOpenUpdate(perangkat.id)}
  ...
  {/* DELETE */}
<button
  onClick={() => handleOpenDelete(perangkat.id)}
...
>
  <div className="relative w-full rounded-3xl bg-white p-6 dark:bg-gray-900">
    ...
    <h4 className="mb-4 text-2xl font-semibold text-red-600">
      Konfirmasi Hapus Perangkat Desa
    </h4>
    ...
    <div className="flex justify-end gap-3 pt-4">
      ...
      <Button
        variant="danger"
        onClick={handleDeletePerangkatDesa}
      >
        Hapus
      </Button>
    </div>
