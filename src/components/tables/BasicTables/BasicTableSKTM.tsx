...
export default function BasicTableSKTM() {
  const [data, setData] = useState<SKTM[]>([]);
  ...
  const [selectedData, setSelectedData] = useState<SKTM | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          "http://127.0.0.1:8000/api/sktm/filter",
          {
            headers: {
  ...
  // === HANDLE UNTUK GET DATA SKTM SETIAP ROLE ===
  const fetchData = async () => {
    try {
       setLoading(true);
    setData([]); // ⛔ cegah render data lama
      ...
      switch (user_type) {
        case "rt":
          apiURL = "http://127.0.0.1:8000/api/rt/sktm/";
          break;
        case "perangkat_desa":
          apiURL = "http://127.0.0.1:8000/api/perangkat_desa/sktm/";
          break;
        case "kepala_desa":
          apiURL = "http://127.0.0.1:8000/api/kepala_desa/sktm/";
          break;
        case "super_admin":
          apiURL = "http://127.0.0.1:8000/api/super_admin/sktm/";
          break;
        default:
          apiURL = "http://127.0.0.1:8000/api/masyarakat/sktm/my";
          break;
      }
      ...
      console.error("Gagal mengambil data Surat Keterangan Tidak Mampu:", error);
      ...
      setAlertData({
        ...
        message:
          "Tidak dapat mengambil data Surat Keterangan Tidak Mampu. Periksa koneksi Anda atau coba beberapa saat lagi.",
      });
 ...
 const handleAjukanKembali = async () => {
  ...
  if (!formUpdate.ttd_masyarakat) {
    newErrors.ttd_masyarakat = "Tanda tangan wajib diupload";
  }
  ...
  try {
    const res = await axios.post(
      `http://127.0.0.1:8000/api/masyarakat/sktm/${selectedData.id_sktm}`,
      fd,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const openModalDetail = (item: SKTM) => {
    setSelectedData(item);
    setModalOpen(true);
  };

  const openModalUpdate = (item: SKTM) => {
  setSelectedData(item);
  setUpdateStatus("");
  ...
  const openModalAjukanKembali = (item: SKTM) => {
    setSelectedData(item);
  ...
  const DEFAULT_POIN_II = `Orang tersebut di atas benar-benar penduduk Desa Mejayan Kecamatan Mejayan Kabupaten Madiun dan keadaan sosial ekonomi orang tersebut di atas benar-benar TIDAK MAMPU`;
  ...
  const openModalUpdateKepala = (item: SKTM) => {
    setSelectedData(item);
  ...
  if (!selectedData) {
    ...
    try {
      ...
      const id_sktm = selectedData.id_sktm;
    ...
    const res = await axios.post(
      `http://127.0.0.1:8000/api/rt/${rt_id}/sktm/${id_sktm}/proses`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    window.location.reload();
      ...
      // refresh data table
      setData((prev) =>
        prev.map((d) => (d.id_sktm === id_sktm ? res.data.data : d))
      );
  ...
  const handleUpdatePerangkat = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://127.0.0.1:8000/api/perangkat-desa/${perangkat_id}/sktm/${id_sktm}/proses`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  if (updateStatusKepala === "selesai" && updateTtdKepala) {
    payload.append("ttd_kades", updateTtdKepala);
      ...
      await axios.post(
        `http://127.0.0.1:8000/api/kepala_desa/${kepalaDesaId}/sktm/${id_sktm}/validasi`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <span className="block text-gray-500 text-theme-xs dark:text-gray-400">
                Pemohon Surat Keterangan Tidak Mampu
              </span>
            </div>
          </div>
        </TableCell>
     ...
     <div className="flex items-center flex-col sm:flex-row justify-between border-t border-gray-200 px-5 py-4 dark:border-gray-800">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="p-3 border rounded-xl bg-gray-50 dark:bg-gray-800 dark:border-gray-700 text-center">
            <p className="text-xs font-bold mb-3 uppercase text-gray-500">Tanda Tangan Pemohon</p>
            {selectedData.ttd_masyarakat ? (
              <div className="flex flex-col items-center">
                ...
                <img 
                  ...
                  alt="TTD" 
                />
                <a href={getFileUrl(selectedData.ttd_masyarakat)} target="_blank" rel="noreferrer" className="text-[11px] text-blue-600 font-bold hover:underline italic">Lihat Tanda Tangan</a>
              </div>
            ) : <p className="text-gray-400 text-xs italic">-</p>}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2.5 pl-1">
          ...
          {selectedData.file_pdf && (
            ...
            >
              ...
              Download Surat Keterangan Tidak Mampu Resmi Terverifikasi (PDF)
            </a>
          )}
        </div>
      </div>
        ...
        <div className="relative flex flex-col w-full max-h-[90vh] overflow-hidden rounded-3xl bg-white dark:bg-gray-900 shadow-xl">
          ...
          <div className="px-6 pt-8 pb-4 lg:px-11 lg:pt-11">
            <h4 className="mb-2 text-2xl font-bold text-gray-800 dark:text-white/90">
              Ajukan Kembali Surat Keterangan Tidak Mampu
            </h4>
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-xl">
    ...
    <div className="mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        <button 
          type="button"
          onClick={() => setUpdateKeteranganKepala("Seluruh persyaratan telah terpenuhi dengan benar, dan Surat Keterangan Tidak Mampu sudah disahkan. Periksa di detail pengajuan surat untuk mengunduh file.")}
          className="px-2 py-1 text-[11px] bg-green-50 text-green-600 border border-green-200 rounded-md hover:bg-green-100 transition"
        >
          + Data Valid
        </button>
