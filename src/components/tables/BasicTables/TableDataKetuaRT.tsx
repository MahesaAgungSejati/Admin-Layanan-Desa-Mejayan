...
import ModalDetailDataKetuaRT from "../../ui/modal/ModalDetailDataKetuaRT";
import ModalUpdateKetuaRT from "../../ui/modal/ModalUpdateKetuaRT";
...
/* ===============================
   COMPONENT
================================ */
export default function TableDataKetuaRT() {
  const [data, setData] = useState<KetuaRT[]>([]);
...
const fetchData = async () => {
  ...
  try {
    const res = await axios.get(
      "http://127.0.0.1:8000/api/users/rt",
      {
        headers: {
    ...
    console.error("Gagal ambil RT:", err);
...
const handleDetail = async (id: number) => {
  try {
    ...
    const res = await axios.get(
      `http://127.0.0.1:8000/api/users/rt/${id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    console.error("Gagal mengambil detail Ketua RT", error);
  }
};
...
if (role === "super_admin" && formData.email) {
  payload.append("email", formData.email);
    ...
    await axios.post(
      `http://127.0.0.1:8000/api/users/rt/${selectedId}`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const handleDeleteRT = async () => {
  ...
  try {
    ...
    await axios.delete(
      `http://127.0.0.1:8000/api/users/rt/${deleteId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    console.error("Gagal menghapus Ketua RT", error);
...
<div className="border-t border-gray-200 dark:border-gray-800 -mx-4 sm:-mx-6 px-4 sm:px-6 pt-4 pb-2 mb-2">
                ...
                <TableCell className="px-5 py-4 sm:px-6 text-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200">
                     <img
                        src={rt.foto_profil || "/images/user/owner.jpg"}
                        alt={rt.nama}
                        onError={(e) => {
  ...
  {/* UPDATE */}
  <button
    onClick={() => handleOpenUpdate(rt.id)}
  ...
  {/* DELETE */}
<button
  onClick={() => handleOpenDelete(rt.id)}
...
>
  <div className="relative w-full rounded-3xl bg-white p-6 dark:bg-gray-900">
    ...
    <h4 className="mb-4 text-2xl font-semibold text-red-600">
      Konfirmasi Hapus Ketua RT
    </h4>

    <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
      Apakah Anda yakin ingin menghapus data Ketua RT ini?
