...
export default function BasicTableSKCK() {
  const [data, setData] = useState<SKCK[]>([]);
  ...
  const [selectedData, setSelectedData] = useState<SKCK | null>(null);
...
const [updateKeteranganSurat, setUpdateKeteranganSurat] = useState("");
const [updateMasaSurat, setUpdateMasaSurat] = useState("");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        "http://127.0.0.1:8000/api/skck/filter",
        {
          headers: {
...
useEffect(() => {
  ...
  // === HANDLE UNTUK GET DATA SKCK SETIAP ROLE ===
  const fetchData = async () => {
    try {
       setLoading(true);
    setData([]); // ⛔ cegah render data lama
      ...
      switch (user_type) {
        case "rt":
          apiURL = "http://127.0.0.1:8000/api/rt/skck/";
          break;
        case "perangkat_desa":
          apiURL = "http://127.0.0.1:8000/api/perangkat_desa/skck/";
          break;
        case "kepala_desa":
          apiURL = "http://127.0.0.1:8000/api/kepala_desa/skck/";
          break;
        case "super_admin":
          apiURL = "http://127.0.0.1:8000/api/super_admin/skck/";
          break;
        default:
          apiURL = "http://127.0.0.1:8000/api/masyarakat/skck/my";
          break;
      }
      ...
      console.error("Gagal mengambil data Pengajuan Permohonan SKCK:", error);
      ...
      setAlertData({
        ...
        message:
          "Tidak dapat mengambil data Pengajuan Permohonan SKCK. Periksa koneksi Anda atau coba beberapa saat lagi.",
      });
 ...
 const handleAjukanKembali = async () => {
  ...
  try {
    const res = await axios.post(
      `http://127.0.0.1:8000/api/masyarakat/skck/${selectedData.id_skck}`,
      fd,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const openModalDetail = (item: SKCK) => {
    setSelectedData(item);
    setModalOpen(true);
  };

  const openModalUpdate = (item: SKCK) => {
  setSelectedData(item);
  setUpdateStatus("");
  ...
  const openModalAjukanKembali = (item: SKCK) => {
    setSelectedData(item);
...
const DEFAULT_KETERANGAN_SURAT = "Adat - istiadat baik";
  ...
  const openModalUpdatePerangkat = (data: any) => {
    ...
    setUpdateKeteranganSurat(
  data.keterangan_surat && data.keterangan_surat.trim() !== ""
    ? data.keterangan_surat
    : DEFAULT_KETERANGAN_SURAT
);

setUpdateMasaSurat(data.masa_surat || "");
  ...
  const openModalUpdateKepala = (item: SKCK) => {
    setSelectedData(item);
  ...
  if (!selectedData) {
    ...
    try {
      ...
      const id_skck = selectedData.id_skck;
    ...
    const res = await axios.post(
      `http://127.0.0.1:8000/api/rt/${rt_id}/skck/${id_skck}/proses`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    window.location.reload();
      ...
      // refresh data table
      setData((prev) =>
        prev.map((d) => (d.id_skck === id_skck ? res.data.data : d))
      );
  ...
  const handleUpdatePerangkat = async () => {
    ...
    if (updateStatusPerangkat && updateStatusPerangkat !== "ditolak") {
     ...
     if (!updateKeteranganSurat || !updateKeteranganSurat.trim()) {
  newErrors.keterangan_surat = "Keterangan surat wajib diisi.";
}

if (!updateMasaSurat || !updateMasaSurat.trim()) {
  newErrors.masa_surat = "Masa surat wajib diisi.";
}
    ...
    try {
      ...
      if (updateStatusPerangkat !== "ditolak") {
        ...
        payload.keterangan_surat = updateKeteranganSurat;
payload.masa_surat = updateMasaSurat;

      }

      await axios.put(
        `http://127.0.0.1:8000/api/perangkat-desa/${perangkat_id}/skck/${id_skck}/proses`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  if (updateStatusKepala === "selesai" && updateTtdKepala) {
    payload.append("ttd_kades", updateTtdKepala);
      ...
      await axios.post(
        `http://127.0.0.1:8000/api/kepala_desa/${kepalaDesaId}/skck/${id_skck}/validasi`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-2xl">
    ...
    <div className="mb-4 border-b pb-3 dark:border-gray-700">
      <h4 className="text-2xl font-bold text-blue-700 dark:text-blue-400">
        Detail Pengajuan Surat Permohonan Pengajuan SKCK
      </h4>
    </div>
  ...
  <div className="flex flex-col gap-2.5 pl-1">
    ...
    {selectedData.file_pdf && (
      ...
      >
        ...
        Download Surat Permohonan Pengajuan SKCK Resmi Terverifikasi (PDF)
      </a>
    )}
  </div>
</div>
        ...
        <div className="relative flex flex-col w-full max-h-[90vh] overflow-hidden rounded-3xl bg-white dark:bg-gray-900 shadow-xl">
          ...
          <div className="px-6 pt-8 pb-4 lg:px-11 lg:pt-11">
            <h4 className="mb-2 text-2xl font-bold text-gray-800 dark:text-white/90">
              Ajukan Kembali Surat Permohonan Pengajuan SKCK
            </h4>
                ...
                <div className="lg:col-span-2 space-y-2">
                  ...
                  {formUpdate.file_ktp && (
                    <div className="mt-3 p-3 border rounded-xl bg-gray-50 dark:bg-gray-800 flex items-center justify-between">
                      ...
                      {formUpdate.file_ktp.type === "application/pdf" ? (
                        <a href={URL.createObjectURL(formUpdate.file_ktp)} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-600 hover:underline">Lihat File</a>
                      ) : (
                ...
                <div className="lg:col-span-2 space-y-2">
                  ...
                  {formUpdate.file_kk && (
                    <div className="mt-3 p-3 border rounded-xl bg-gray-50 dark:bg-gray-800 flex items-center justify-between">
                      ...
                      {formUpdate.file_kk.type === "application/pdf" ? (
                        <a href={URL.createObjectURL(formUpdate.file_kk)} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-600 hover:underline">Lihat File</a>
                      ) : (
                        <span className="text-[10px] text-green-600 font-medium">✓ Gambar Terpilih</span>
                      )}
                    </div>
...
<Modal
  ...
  className="max-w-xl w-full m-4"
>
  {/* Container utama dengan Flex dan Max Height agar bisa diatur scrollnya */}
  <div className="relative flex flex-col w-full max-h-[90vh] overflow-hidden bg-white rounded-2xl dark:bg-gray-900 shadow-xl">
    ...
    <div className="flex-grow overflow-y-auto px-6 pb-2 custom-scrollbar">
      ...
      {updateStatusPerangkat !== "ditolak" && (
        <div className="mb-4">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <button
              type="button"
              onClick={() => setUpdateKeteranganSurat(DEFAULT_KETERANGAN_SURAT)}
          ...
          <textarea
            value={updateKeteranganSurat}
            onChange={(e) => {
              setUpdateKeteranganSurat(e.target.value);
              setErrorsPerangkat((p) => ({ ...p, keterangan_surat: undefined }));
            }}
            className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm bg-gray-50 dark:bg-gray-800"
            rows={2}
            placeholder="Contoh: Adat - istiadat baik"
          />

          {errorsPerangkat.keterangan_surat && (
            <p className="text-xs text-red-600 mt-1">
              {errorsPerangkat.keterangan_surat}
            </p>
          )}
      ...
      {updateStatusPerangkat !== "ditolak" && (
        <div className="mb-4">
          <label className="block mb-1.5 text-sm font-semibold text-gray-700 dark:text-gray-300">
            Masa Surat <span className="text-red-500">*</span>
          </label>

          <input
            type="text"
            value={updateMasaSurat}
            onChange={(e) => {
              setUpdateMasaSurat(e.target.value);
              setErrorsPerangkat((p) => ({ ...p, masa_surat: undefined }));
            }}
            className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"
            placeholder="Contoh: Berlaku selama 3 bulan sejak tanggal diterbitkan"
          />

          {errorsPerangkat.masa_surat && (
            <p className="text-xs text-red-600 mt-1">
              {errorsPerangkat.masa_surat}
            </p>
          )}
    ...
    {/* ACTION BUTTONS - Tetap (Fixed) */}
    <div className="flex justify-end gap-3 border-t p-6 bg-gray-50 dark:bg-gray-800/50">
      <button
        type="button"
        className="px-5 py-2 text-sm border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50 transition"
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-xl">
    ...
    <div className="mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        <button 
          type="button"
          onClick={() => setUpdateKeteranganKepala("Seluruh persyaratan telah terpenuhi dengan benar, dan Surat Permohonan Pengajuan SKCK sudah disahkan. Periksa di detail pengajuan surat untuk mengunduh file.")}
          className="px-2 py-1 text-[11px] bg-green-50 text-green-600 border border-green-200 rounded-md hover:bg-green-100 transition"
        >
          + Data Valid
        </button>
