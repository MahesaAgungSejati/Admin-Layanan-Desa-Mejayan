...
import ModalDetailDataKepalaDesa from "../../ui/modal/ModalDetailDataKepalaDesa";
import ModalUpdateKepalaDesa from "../../ui/modal/ModalUpdateKepalaDesa";
...
/* ===============================
   COMPONENT
================================ */
export default function TableDataKepalaDesa() {
...
const fetchData = async () => {
  ...
  try {
    const res = await axios.get(
      "http://127.0.0.1:8000/api/users/kepala-desa",
      {
        headers: {
    ...
    console.error("Gagal ambil Kepala Desa:", err);
...
const handleDetail = async (id: number) => {
  try {
    ...
    const res = await axios.get(
      `http://127.0.0.1:8000/api/users/kepala-desa/${id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    console.error("Gagal mengambil detail Kepala Desa", error);
  }
};
...
const handleOpenUpdate = async (id: number) => {
  try {
    ...
    setSelectedId(id);
    setFormData(res.data.data); // isi form
    setOpenUpdate(true);
  } catch (error) {
    console.error("Gagal mengambil data Kepala Desa", error);
  }
};
...
if (role === "super_admin" && formData.email) {
  payload.append("email", formData.email);
    ...
    await axios.post(
      `http://127.0.0.1:8000/api/users/kepala-desa/${selectedId}`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const handleDeleteKepalaDesa = async () => {
  if (!deleteId) return;
  ...
  try {
    ...
    await axios.delete(
      `http://127.0.0.1:8000/api/users/kepala-desa/${deleteId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    console.error("Gagal menghapus Kepala Desa", error);
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        message:
          error?.response?.data?.message ??
          "Gagal menghapus data Kepala Desa.",
      })
    );
...
<div className="border-t border-gray-200 dark:border-gray-800 -mx-4 sm:-mx-6 px-4 sm:px-6 pt-4 pb-2 mb-2">
      ...
      <div className="max-w-full overflow-x-auto">
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {paginatedData.map((kepalaDesa) => (
              <TableRow key={kepalaDesa.id}>
                {/* FOTO + NAMA + EMAIL */}
                <TableCell className="px-5 py-4 sm:px-6 text-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200">
                     <img
                        src={kepalaDesa.foto_profil || "/images/user/owner.jpg"}
                        alt={kepalaDesa.nama}
                        onError={(e) => {
  ...
  {/* DETAIL */}
  <button
    onClick={() => handleDetail(kepalaDesa.id)}
  ...
  {/* UPDATE */}
  <button
    onClick={() => handleOpenUpdate(kepalaDesa.id)}
...
{role === "super_admin" && (
  <button
    onClick={() => handleOpenDelete(kepalaDesa.id)}
...
>
  <div className="relative w-full rounded-3xl bg-white p-6 dark:bg-gray-900">
    ...
    <h4 className="mb-4 text-2xl font-semibold text-red-600">
      Konfirmasi Hapus Kepala Desa
    </h4>

    <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
      Apakah Anda yakin ingin menghapus data Kepala Desa ini?
    ...
    <div className="flex justify-end gap-3 pt-4">
      ...
      <Button
        variant="danger"
        onClick={handleDeleteKepalaDesa}
      >
        Hapus
      </Button>
    </div>
