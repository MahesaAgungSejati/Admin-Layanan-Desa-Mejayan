...
export default function BasicTableSKJ() {
  const [data, setData] = useState<SKJ[]>([]);
  ...
  const [selectedData, setSelectedData] = useState<SKJ | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        "http://127.0.0.1:8000/api/skj/filter",
        {
          headers: {
...
useEffect(() => {
  ...
  // === HANDLE UNTUK GET DATA SKJ SETIAP ROLE ===
  const fetchData = async () => {
    try {
       setLoading(true);
    setData([]); // ⛔ cegah render data lama
      ...
      switch (user_type) {
        case "rt":
          apiURL = "http://127.0.0.1:8000/api/rt/skj/";
          break;
        case "perangkat_desa":
          apiURL = "http://127.0.0.1:8000/api/perangkat_desa/skj/";
          break;
        case "kepala_desa":
          apiURL = "http://127.0.0.1:8000/api/kepala_desa/skj/";
          break;
        case "super_admin":
          apiURL = "http://127.0.0.1:8000/api/super_admin/skj/";
          break;
        default:
          apiURL = "http://127.0.0.1:8000/api/masyarakat/skj/my";
          break;
      }
      ...
      console.error("Gagal mengambil data SKJ:", error);
      ...
      setAlertData({
        ...
        message:
          "Tidak dapat mengambil data SKJ. Periksa koneksi Anda atau coba beberapa saat lagi.",
      });
 ...
 const handleAjukanKembali = async () => {
  ...
  if (!formUpdate.nama_mantan_suami) newErrors.nama_mantan_suami = "Nama mantan suami wajib diisi";
  ...
  fd.append("nama_mantan_suami", formUpdate.nama_mantan_suami);
  ...
  try {
    const res = await axios.post(
      `http://127.0.0.1:8000/api/masyarakat/skj/${selectedData.id_skj}`,
      fd,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const openModalDetail = (item: SKJ) => {
    setSelectedData(item);
    setModalOpen(true);
  };

  const openModalUpdate = (item: SKJ) => {
  setSelectedData(item);
  setUpdateStatus("");
  ...
  const openModalAjukanKembali = (item: SKJ) => {
    setSelectedData(item);
  ...
  setFormUpdate({
    ...
    alasan: item.alasan ?? "",
    nama_mantan_suami: item.nama_mantan_suami ?? "",
    keterangan: "",
  ...
  const getDefaultPoinII = (mantanSuami?: string) => {
  return `Orang tersebut di atas benar-benar penduduk Desa Mejayan Kecamatan  Mejayan Kabupaten  Madiun dan orang tersebut benar-benar berstatus janda dari suami yang bernama ${
    mantanSuami && mantanSuami.trim() !== ""
      ? mantanSuami
      : "........"
  }`;
};
  ...
  data.poin_ii && data.poin_ii.trim() !== ""
    ? data.poin_ii
    : getDefaultPoinII(data.nama_mantan_suami)
);
  ...
  const openModalUpdateKepala = (item: SKJ) => {
    setSelectedData(item);
  ...
  if (!selectedData) {
    ...
    try {
      ...
      const id_skj = selectedData.id_skj;
    ...
    const res = await axios.post(
      `http://127.0.0.1:8000/api/rt/${rt_id}/skj/${id_skj}/proses`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    window.location.reload();
      ...
      // refresh data table
      setData((prev) =>
        prev.map((d) => (d.id_skj === id_skj ? res.data.data : d))
      );
  ...
  const handleUpdatePerangkat = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://127.0.0.1:8000/api/perangkat-desa/${perangkat_id}/skj/${id_skj}/proses`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  if (updateStatusKepala === "selesai" && updateTtdKepala) {
    payload.append("ttd_kades", updateTtdKepala);
      ...
      await axios.post(
        `http://127.0.0.1:8000/api/kepala_desa/${kepalaDesaId}/skj/${id_skj}/validasi`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <span className="block text-gray-500 text-theme-xs dark:text-gray-400">
                Pemohon Surat Keterangan Janda
              </span>
            </div>
          </div>
        </TableCell>
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-2xl">
    ...
    <div className="mb-4 border-b pb-3 dark:border-gray-700">
      <h4 className="text-2xl font-bold text-blue-700 dark:text-blue-400">
        Detail Pengajuan Surat Keterangan Janda
      </h4>
    </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-8">
              {[
                ...
                { label: "Nama Mantan Suami", value: selectedData.nama_mantan_suami },
  ...
  <div className="flex flex-col gap-2.5 pl-1">
    ...
    {selectedData.file_pdf && (
      ...
      >
        ...
        Download Surat Keterangan Janda Resmi Terverifikasi (PDF)
      </a>
    )}
  </div>
</div>
        ...
        <div className="relative flex flex-col w-full max-h-[90vh] overflow-hidden rounded-3xl bg-white dark:bg-gray-900 shadow-xl">
          ...
          <div className="px-6 pt-8 pb-4 lg:px-11 lg:pt-11">
            <h4 className="mb-2 text-2xl font-bold text-gray-800 dark:text-white/90">
              Ajukan Kembali Surat Keterangan Janda
            </h4>
          ...
          <form className="flex flex-col overflow-hidden">
            <div className="custom-scrollbar flex-grow overflow-y-auto px-6 pb-6 lg:px-11">
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label>Nama Mantan Suami</Label>
                  <Input
                    type="text"
                    value={formUpdate.nama_mantan_suami}
                    onChange={(e) => setFormUpdate({ ...formUpdate, nama_mantan_suami: e.target.value })}
                  />
                  {errors.nama_mantan_suami && <p className="mt-1 text-xs text-red-500">{errors.nama_mantan_suami}</p>}
                </div>
                ...
                <div className="lg:col-span-2 space-y-2">
                  ...
                  <TextArea
                    ...
                    placeholder="Berikan alasan anda mengajukan surat..."
                  />
      ...
      <div className="mb-4">
        <div className="flex justify-between items-center mb-1.5">
          ...
          <button
            type="button"
            onClick={() =>
  setUpdatePoinII(getDefaultPoinII(selectedData?.nama_mantan_suami))
}
  ...
  <div className="relative w-full p-6 bg-white rounded-2xl dark:bg-gray-900 shadow-xl">
    ...
    <div className="mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        <button 
          type="button"
          onClick={() => setUpdateKeteranganKepala("Seluruh persyaratan telah terpenuhi dengan benar, dan Surat Keterangan Janda sudah disahkan. Periksa di detail pengajuan surat untuk mengunduh file.")}
          className="px-2 py-1 text-[11px] bg-green-50 text-green-600 border border-green-200 rounded-md hover:bg-green-100 transition"
        >
          + Data Valid
        </button>
